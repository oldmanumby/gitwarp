...
describe('Standard Trick Cards & Compatibility System', () => {
  ...
  describe('Catalog Completeness', () => {
    ...
    it('has all required 11 existing and 12 new card IDs', () => {
      const expectedIds = [
        'boltnew', 'deepwiki', 'gitdiagram', 'gitingest', 'githubdev',
        'githubgg', 'github1s', 'gitmcp', 'gitpodcast', 'stackblitz', 'starhistory',
    ...
    it('every card has valid metadata and generateUrl function', () => {
      for (const card of STANDARD_CARDS) {
        ...
        assert.ok(typeof card.name === 'string' && card.name.length > 0, `Card missing name: ${card.id}`);
        assert.ok(typeof card.icon === 'string' && card.icon.length > 0, `Card missing icon: ${card.id}`);
        assert.ok(Array.isArray(card.allowedContexts) && card.allowedContexts.length > 0, `Card missing allowedContexts: ${card.id}`);
        assert.ok(typeof card.description === 'string' && card.description.length > 0, `Card missing description: ${card.id}`);
  ...
  describe('Compatibility Across Contexts', () => {
    ...
    describe('User Context (https://github.com/torvalds)', () => {
      ...
      it('the other 21 repo/file/commit/PR cards are incompatible with User context', () => {
        ...
        for (const card of incompatibleCards) {
          assert.equal(isCardCompatible(card, userCtx), false, `Card ${card.id} should be incompatible with User context`);
          assert.equal(getCardUrl(card, userCtx), null, `Card ${card.id} should return null URL for User context`);
        }
      });
    });
    ...
    describe('File Context (https://github.com/facebook/react/blob/main/package.json)', () => {
      ...
      it('raw_file is compatible and generates correct raw URL', () => {
        ...
        assert.equal(getCardUrl(rawFileCard, fileCtx), 'https://raw.githubusercontent.com/facebook/react/main/package.json');
      });
      ...
      it('deep link aware cards generate deep file links', () => {
        ...
        assert.equal(getCardUrl(gitingest, fileCtx), 'https://gitingest.com/facebook/react/blob/main/package.json');
        assert.equal(getCardUrl(githubdev, fileCtx), 'https://github.dev/facebook/react/blob/main/package.json');
        assert.equal(getCardUrl(stackblitz, fileCtx), 'https://stackblitz.com/github/facebook/react?file=package.json');
      });
    });
    ...
    describe('Commit Context (https://github.com/facebook/react/commit/a1b2c3d)', () => {
      ...
      it('.patch and .diff are compatible and generate commit patch/diff URLs', () => {
        ...
        assert.equal(getCardUrl(patchCard, commitCtx), 'https://github.com/facebook/react/commit/a1b2c3d.patch');
        assert.equal(getCardUrl(diffCard, commitCtx), 'https://github.com/facebook/react/commit/a1b2c3d.diff');
      });
      ...
      it('deep link aware cards generate commit deep links', () => {
        ...
        assert.equal(getCardUrl(gitingest, commitCtx), 'https://gitingest.com/facebook/react/commit/a1b2c3d');
        assert.equal(getCardUrl(githubdev, commitCtx), 'https://github.dev/facebook/react/commit/a1b2c3d');
        assert.equal(getCardUrl(gitpod, commitCtx), 'https://gitpod.io/#https://github.com/facebook/react/commit/a1b2c3d');
      });
    });
    ...
    describe('PR Context (https://github.com/facebook/react/pull/42)', () => {
      ...
      it('.patch and .diff are compatible and generate PR patch/diff URLs', () => {
        ...
        assert.equal(getCardUrl(patchCard, prCtx), 'https://github.com/facebook/react/pull/42.patch');
      ...
      it('deep link aware cards generate PR deep links', () => {
        ...
        assert.equal(getCardUrl(vscodeDev, prCtx), 'https://vscode.dev/github/facebook/react/pull/42');
      });
    });
    ...
    describe('Unknown / Invalid Context', () => {
      ...
      it('returns isCardCompatible === false for all 23 cards', () => {
        ...
        for (const card of STANDARD_CARDS) {
          assert.equal(isCardCompatible(card, unknownCtx), false, `Card ${card.id} should be incompatible with Unknown context`);
          assert.equal(getCardUrl(card, unknownCtx), null, `Card ${card.id} should return null URL for Unknown context`);
        }
      });
  ...
  describe('URL Generation Accuracy for All 23 Cards', () => {
    ...
    it('handles branch ref in commits.atom and zip_archive', () => {
      ...
      assert.equal(getCardUrl(commitsAtom, branchCtx), 'https://github.com/octocat/Hello-World/commits/dev.atom');
      assert.equal(getCardUrl(zipArchive, branchCtx), 'https://github.com/octocat/Hello-World/archive/refs/heads/dev.zip');
    });
  });
